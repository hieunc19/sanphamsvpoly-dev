...
@section('title')
    <title>Giảng Viên Hướng Dẫn Phê Duyệt</title>
@endsection
@section('content')
<div class="container">
            ...
            <table class="table table-bordered table-checkable dataTable no-footer dtr-inline collapsed" id="kt_datatable" role="grid" aria-describedby="kt_datatable_info">
              <thead>
                <tr role="row">
                  ...
                  <th class="sorting" tabindex="0" aria-controls="kt_datatable" rowspan="1" colspan="1" >Mã</th>
                  <th class="sorting" tabindex="0" aria-controls="kt_datatable" rowspan="1" colspan="1" >GVHD</th>
                  <th class="sorting" tabindex="0" aria-controls="kt_datatable" rowspan="1" colspan="1" >Kỳ Học</th>
                  <th class="sorting" tabindex="0" aria-controls="kt_datatable"  >Trạng thái đánh giá</th>
                  <th style="width:220px" class="sorting" tabindex="0" aria-controls="kt_datatable" rowspan="1" colspan="1" >Đánh giá</th>
                  <th class="sorting" tabindex="0" aria-controls="kt_datatable" rowspan="1" colspan="1" >Trạng Thái</th>
                  <th style="width:200px" class="sorting" tabindex="0" aria-controls="kt_datatable" rowspan="1" colspan="1" >Lý do từ chối</th>
              ...
              <tbody id="list-content">
                @foreach ($product as $prod)
                  ...
                  <tr>
                    ...
                    <td>{{$prod->evaluate_status == 1 ? "Chưa đánh giá" : "Đã đánh giá"}}</td>
                    <td>
                      <ol>
                        @foreach ($prod->product_evaluates as $pe)
                          <li>{{$pe->name}} - {!!$pe->display_evaluate($prod->id)!!}</li>
                        @endforeach
                      </ol>
                    </td>
                    <td>@if ($prod->status==2)
                        <span style="color: red; font-weight: 700">Sản Phẩm Bị Từ Chối</span>
                        @else 
                        <span style="color: green; font-weight: 700">Sản Phẩm Mới Được Tạo</span>
                        @endif
                    </td>
                    <td>{{$prod->reject_reason}}</td>
                    <td style="display: grid-flex; text-align: center">
                      <div class="btn-group dropright">
                        <button type="button" class="btn btn-info dropdown-toggle " data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        </button>
                        <div class="dropdown-menu " style="text-align: center;" >
                            <a href="{{route('detail.product.view', ['token' => $prod->token])}}"
                              class="btn btn-info mb-2" target="_blank"  @popper(Xem Trước)
                              >
                              <i class="fa fa-file"></i>
                            </a>
                            <a href="{{route('preview/evaluate', ['id' => $prod->id])}}" class="btn btn-primary mb-2" @popper(Đánh giá)><i class="fas	fa-star"></i></a>  
                            <a href="{{route('edit.product', ['token' => $prod->token])}}" class="btn btn-primary mb-2" @popper(Chỉnh Sửa)><i class="fa	fa-edit"></i></a>  
                            <a onclick="return confirm('Bạn Có Muốn Phê Duyệt Sản Phẩm Này Không??')" href="{{route('preview/accept', ['id' => $prod->id])}}" class="btn btn-primary mb-2" @popper(Phê Duyệt)><i class="fa	fa-check"></i></a>
                            <button id="reject_modal" class="reject_modal btn btn-danger mb-2" data-id="{{$prod->id}}" data-url="{{route('preview/reject')}}" @popper(Từ Chối)><i class=" fa fa-ban"></i></button>
                            <a  class=" btn btn-danger mb-2" href="{{route('preview/del', ['id' => $prod->id])}}" onclick="return confirm('Bạn Có Muốn Xóa Sản Phẩm Này Không')"  @popper(Xóa)><i class="fa fa-trash-alt"></i></a>
                        </div>
                      </div>  
                    </td>
                  </tr>
