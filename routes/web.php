...
use App\Http\Controllers\SubjectAttributeController;
...
Route::middleware('auth')->group(function () {
    ...
    Route::prefix('admin')->middleware('role:teacher|major_head_teacher|admin|giao_vu')->group(function () {
        ...
        //import Excel
        Route::get('nhap-diem', [DashboardController::class, 'nhapdiem'])->name('nhap-diem')->middleware('role:giao_vu|admin');
        Route::post('nhap-diem', [ExcelController::class, 'import'])->name('nhap-diem')->middleware('role:giao_vu|admin');

        //Giáo Viên Hướng Dẫn Xem Trước/PHê Duyệt/Từ Chối
        Route::get('preview', [TeacherPreviewController::class, 'preview'])->name('preview')->middleware('role:teacher|admin');
        Route::post('preview', [TeacherPreviewController::class, 'ajaxpreview'])->name('ajaxpreview');
        Route::any('preview/{id}/accept', [TeacherPreviewController::class, 'accept'])->name('preview/accept')->middleware('role:teacher|admin');
        Route::any('preview/{id}/evaluate', [TeacherPreviewController::class, 'evaluate'])->name('preview/evaluate')->middleware('role:teacher|admin');
        Route::get('preview/reject', [TeacherPreviewController::class, 'rejectModal'])->name('preview/reject')->middleware('role:teacher|admin');
        Route::post('preview/reject', [TeacherPreviewController::class, 'reject'])->name('preview/reject')->middleware('role:teacher|admin');
        Route::any('preview/{id}/del', [TeacherPreviewController::class, 'del'])->name('preview/del')->middleware('role:teacher|admin');
        Route::post('product-evaluate/update', [TeacherPreviewController::class, 'updateEvaluate'])->name('preview.evaluate')->middleware('role:teacher|admin');
        //demo
        ...
        Route::get('subject-attr', [SubjectAttributeController::class, 'getAttribute'])->name('subject_attribute.find_by_id');
        Route::post('subject-attr/check-name', [SubjectAttributeController::class, 'checkName'])->name('subject_attribute.check_name');
        Route::post('subject-attr/save-attr', [SubjectAttributeController::class, 'saveAttr'])->name('subject_attribute.save');
        Route::post('subject-attr/remove-attr', [SubjectAttributeController::class, 'removeAttr'])->name('subject_attribute.remove');
