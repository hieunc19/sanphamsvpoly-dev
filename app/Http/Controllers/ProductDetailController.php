...
class ProductDetailController extends Controller
{
    public function index ($token) {
        ...
        if (isset($product)) {
            ...
            $evaluates = ProductEvaluate::select('product_evaluations.*', 'subject_attributes.name as attribute_name')
                        ->join('subject_attributes', 'subject_attributes.id', '=', 'product_evaluations.attribute_id')
                        ->where('product_evaluations.product_id', $product->id)
                        ->get();

                        //cắt chuỗi
            if (!$product->url_video) {
                $url_video = '';
            } else if ($product->status_video==2) {    //youtube
